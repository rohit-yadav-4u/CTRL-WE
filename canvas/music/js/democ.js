
{
    ...
    let allowTilt = true;
    // Only run if .trad-music section exists
    const tradMusicSection = document.querySelector('.trad-music');
    if (tradMusicSection) {
        const gridEl = tradMusicSection.querySelector('.grid');
        if (gridEl) {
            new Grid(gridEl);
            imagesLoaded(gridEl.querySelectorAll('.box__img'), () => document.body.classList.remove('loading'));
        }
    }
}
