...
(function() {
	...
	function Book( el ) {
		this.el = el;
		this.book = this.el.querySelector( '.library .book' );
		this.ctrls = this.el.querySelector( '.library .buttons' );
		this.details = this.el.querySelector( '.library .details' );
	...
	Book.prototype._initBookBlock = function() {
		...
		this.bb = new BookBlock( this.bbWrapper.querySelector( '.library .bb-bookblock' ), {
			speed : 700,
			shadowSides : 0.8,
			shadowFlip : 0.4
		} );
		// book controls
		this.ctrlBBClose = this.bbWrapper.querySelector( '.library .bb-nav-close' );
		this.ctrlBBNext = this.bbWrapper.querySelector( '.library .bb-nav-next' );
		this.ctrlBBPrev = this.bbWrapper.querySelector( '.library .bb-nav-prev' );
	}
	...
	Book.prototype._initEvents = function() {
		...
		if( this.details ) {
			this.ctrls.querySelector( '.library a:nth-child(2)' ).addEventListener( 'click', function( ev ) { ev.preventDefault(); self._showDetails(); } );
